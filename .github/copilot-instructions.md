...
Real-world bugs found during testing are logged in ISSUES.md at the project root. Always read ISSUES.md before modifying any file — it documents root causes and fixes to avoid regressions.

Key rules:

- When a bug is fixed, add it to ISSUES.md immediately
- Include: file affected, symptom, root cause, fix, commit reference
- ISSUES.md is local-only and listed in .gitignore — never commit or push it
...
- Must use `pvesh get /cluster/nextid` to suggest next free VMID
- Must validate VMID by checking file existence:
  `/etc/pve/qemu-server/${VMID}.conf and /etc/pve/lxc/${VMID}.conf`
